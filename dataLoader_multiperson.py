...
class val_loader(object):
    ...
    def __getitem__(self, index):
        ...
        target_video = self.mixLst[index]
        # print(f"[DEBUG] target_video line: {repr(target_video)}")
        data = target_video.split('\t')
        ...
        if videoName not in self.ts_to_entity:
            print(f"[ERROR] videoName '{videoName}' not found in val_entity.json.")
            print(f"[DEBUG] Available keys (first 5): {list(self.ts_to_entity.keys())[:5]}")
            raise KeyError(f"videoName '{videoName}' missing.")
        all_ts = list(self.entity_data[videoName][target_entity].keys())
        numFrames = int(data[1])
